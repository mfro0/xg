...
FT_Evnt_send_MSB (CLIENT * clnt, WINDOW * wind, CARD16 evnt, va_list vap)
{
    ...
    if (evn) {
        ...
        // va_arg() cannot pass anything else than int (integer promotion)
        #define ARG(t)   { *(t*)ptr = (t) va_arg (vap, int); ptr += sizeof(t); }
...
FT_Evnt_send_LSB (CLIENT * clnt, WINDOW * wind, CARD16 evnt, va_list vap)
{
    ...
    if (evn) {
        ...
        #define ARG32(t) { *(t*)ptr = Swap32(va_arg (vap, int)); ptr += 4; }
        #define ARG16(t) { *(t*)ptr = Swap16(va_arg (vap, int)); ptr += 2; }
        ...
        while (*frm) switch (*(frm++)) {
            ...
            case 'c': ARG   (CARD8);    break;
            case 'b': ARG   (BOOL);     break;
            // FIXME: case 'p': SwapPXY ((PXY*)ptr,  &va_arg (vap, PXY));    ptr += 4; break;
